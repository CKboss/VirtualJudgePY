/* http://prismjs.com/download.html?themes=prism&languages=markup+css+clike+javascript+c+csharp+cpp+haskell+java+markdown+pascal+python&plugins=line-numbers+normalize-whitespace */
                    ...
                    for (var p = 0; p < r.length; p++) {
                        ...
                        if (!(m instanceof a)) {
                            ...
                            if (!v && h && p != r.length - 1) {
                                ...
                                if (p < r.length - 2 && (k += r[p + 2].matchedStr || r[p + 2]), u.lastIndex = 0, v = u.exec(k), !v)continue;
                                var w = v.index + (g ? v[1].length : 0);
                                if (w >= m.length)continue;
                                var _ = v.index + v[0].length, P = m.length + b.length;
                                y = 3, P >= _ && (y = 2, k = k.slice(0, P)), m = k
                            }
                            if (v) {
                                g && (f = v[1].length);
                                var w = v.index + f, v = v[0].slice(f), _ = w + v.length, S = m.slice(0, w), O = m.slice(_), j = [p, y];
                                S && j.push(S);
