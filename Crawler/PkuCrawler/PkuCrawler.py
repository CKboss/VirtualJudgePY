...
class PkuCrawler():
    ...
    def getDetail(self, html):
        ...
        d['timelimit'] = soup.select_one(
            'body > table > tbody > tr > td > div.plm > table > tbody > tr:nth-of-type(1) > td:nth-of-type(1)').text.replace(
            'Time Limit:', '')
        d['memorylimit'] = soup.select_one(
            'body > table > tbody > tr > td > div.plm > table > tbody > tr:nth-of-type(1) > td:nth-of-type(3)').text.replace(
            'Memory Limit:', '')
        if soup.select_one(
                'body > table > tbody > tr > td > div.plm > table > tbody > tr:nth-of-type(2) > td:nth-of-type(5)') is None:
            d['specialjudge'] = 0
        else:
            d['specialjudge'] = 1
